...
<head>
    ...
    <title>View Players - Cricket Academy</title>
            ...
            <div class="container-fluid">
                ...
                <div class="card shadow">
                    <div class="card-header py-3">
                        <p class="text-primary m-0 fw-bold" style="border-color: #009977;color: #009977;">Players in Cricket Academy</p>
                    </div>
                    <div class="card-body">
                       <div>
                           <?php
                           $conn = mysqli_connect('localhost','root','','cricket_academy');
                           $query = "SELECT user.user_id, user.name, user.email, user.date_of_birth, user.nationality, user.age, player.role, player.batting_style, player.bowling_style, (SELECT user.name FROM user WHERE user_id=player.coach_id) AS Coach FROM user JOIN player ON user.user_id=player.user_id;";
                           ...
                           <table class="table table-borderless">
                               ...
                               <?php while($row = mysqli_fetch_array($result)) { ?>
                                   <tr border = '2px solid black'>
                                       ...
                                       <td>
                                           <a href="process_delete_player.php?del=<?php echo $row['user_id']; ?>" class="del_btn">Delete</a>
                                       </td>
                                   </tr>
                               <?php } ?>
                       </div>
